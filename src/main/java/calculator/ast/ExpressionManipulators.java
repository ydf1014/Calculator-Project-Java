...
public class ExpressionManipulators {
    ...
    private static double toDoubleHelper(IDictionary<String, AstNode> variables, AstNode node) {
        ...
        if (node.isNumber()) {
           ...
           AstNode varChild = variables.get(name);
           if (varChild.isNumber()) {
               return varChild.getNumericValue();
           } else {
               return toDoubleHelper(variables, new AstNode(varChild.getName(), varChild.getChildren()));
           }
        } else {
            ...
            if (children.size() != 1) {
                ...
                if (name.equals("+")) {
                    ...
                    if (toDoubleHelper(variables, second) == 0) {
                        throw new EvaluationError("Result is undefined");
                    } else {
                        return toDoubleHelper(variables, first) / toDoubleHelper(variables, second);
                    }
                } else {
                    throw new EvaluationError("Expression contains an undefined operation");
                }
            } else {
                AstNode only = children.get(0);
                if (name.equals("negate")) {
                    return -toDoubleHelper(variables, only);
                } else if (name.equals("sin")) {
                    return Math.sin(toDoubleHelper(variables, only));
                } else if (name.equals("cos")) {
                    return Math.cos(toDoubleHelper(variables, only));
                } else {
                    throw new EvaluationError("Expression contains an undefined operation ");
                }
            }
    ...
    public static AstNode handleSimplify(Environment env, AstNode node) {
        ...
        AstNode simplifiedVar = simplifyVariables(env, node.getChildren().get(0));
        return simplifyHelper(env, simplifiedVar);
    }
    
    private static AstNode simplifyHelper(Environment env, AstNode node) {
        IList<AstNode> children = node.getChildren();
        if (node.isOperation() && 
           (node.getName().equals("+") || node.getName().equals("-") || node.getName().equals("*"))) {
    ...
    private static AstNode simplifyVariables(Environment env, AstNode node) {
        if (node.isOperation()) {
            for (int i = 0; i < node.getChildren().size(); i++) {
                node.getChildren().set(i, simplifyVariables(env, node.getChildren().get(i)));
            }
        } 

        if (node.isVariable() && env.getVariables().containsKey(node.getName())) {
            ...
            for (int i = 0; i < var.getChildren().size(); i++) {
                AstNode child = var.getChildren().get(i);
                if (child.isNumber()) {
                    newChildren.add(simplifyVariables(env, new AstNode(child.getNumericValue())));
                } 
                if (child.isVariable()){
                    newChildren.add(simplifyVariables(env, new AstNode(child.getName())));
                } 
                if (child.isOperation()){
                    newChildren.add(simplifyVariables(env, new AstNode(child.getName(), child.getChildren())));
                } 
            }
            return newNode;
        }
    ...
    */
    public static AstNode plot(Environment env, AstNode node) {
        ...
        AstNode line = childList.get(0);
        String var = childList.get(1).getName();
        double min = toDoubleHelper(variables, childList.get(2));
        double max = toDoubleHelper(variables, childList.get(3));
        double step = toDoubleHelper(variables, childList.get(4));
            
        if (env.getVariables().containsKey(var)) {
            throw new EvaluationError("var is defined: " + var);
        } else if (max < min) {
            throw new EvaluationError("max < min: " + max + " < " + min);
        } else if (step <= 0) {
            throw new EvaluationError("step <= 0: " + step);
        }
            
        while (min <= max) {
            env.getVariables().put(var, new AstNode(min));
            xValues.add(min);
            yValues.add(toDoubleHelper(variables, line));
            min += step;
                
        }
        env.getVariables().remove(var);
        env.getImageDrawer().drawScatterPlot("plot", var, "output", xValues, yValues);
